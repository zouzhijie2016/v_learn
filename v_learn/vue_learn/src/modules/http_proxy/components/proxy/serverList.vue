...
export default {
    ...
    methods: {
        ...
        async loadNode (node, resolve) {
            ...
            if (node.level === 1) {
                const data = await nodeApi.getNode(node.data.Node)
                const services = []
                const servicesNode = data.Node
                for (const serviceKey in data.Services) {
                    const service = data.Services[serviceKey]
                    service.nodeMeta = servicesNode.Meta
                    services.push(service)
                }
                resolve(services)
            }
            if (node.level === 2) {
                const data = await nodeApi.getService(node.data.Service)
                const serviceArr = []
                for (const dataKey in data) {
                    const service = data[dataKey]
                    if (service.ServiceID === node.data.ID) {
