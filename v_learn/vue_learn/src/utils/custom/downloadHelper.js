...
function jsonToExcelConvertor (JSONData, showLabel, type) {
    type = type ? type : 'xls'
    ...
    var tableHeaderRow = generateRow(showLabel)
    if(arrData && arrData.length) {
        arrData.forEach(item=>{
            tableContentRows += generateRow(item);
        })
    }
    var excelFile = `
    ...
    return 'data:' + application + ';charset=utf-8,' + encodeURIComponent(excelFile)
}