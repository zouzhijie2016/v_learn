<template>
  <div>
      <el-dialog :title="title" custom-class="submitDialog" :visible.sync="dataCloseVisible" :close-on-click-modal="false" :close-on-press-escape="false">
          <div>
              <el-form :model="form" label-position="left" ref="form" label-width="165px" v-loading="loading" element-loading-text="Loading...">
                  <div class="confirmTips" v-if="extendOptions.dataType == 'Submit'">Are you sure to submit this process?</div>
                  <el-form-item label="Status : " class="submit_dialog_item" v-if="extendOptions.dataType == 'View'">
                  ...
                  <el-form-item label="Audit Note : " class="submit_dialog_item">
                      <div style="word-break: break-all;width: 100%;line-height: 20px">{{ form.auditNote }}</div>
                  </el-form-item>
              </el-form>
              <div class="submit_dialog_btns" v-if="extendOptions.dataType == 'Submit'">
                  <el-button type="primary" @click="submitDialogEvt" :loading="submitLoading">Submit</el-button>
...
export default {
    ...
    methods: {
        submitDialogEvt () {
            var t = this
            var params = Object.assign({}, this.fieldData, {
                isDrafted: -1
            })

            t.submitLoading = true
            ReportApi.createOrEditSheet(params).then(d => {
                t.submitLoading = false
                if (d.success) {
                    this.getRedPointerList()

                    t.$showSuccessMessage('Submitted success!')
        ...
        setParmarsData (data) {
            ...
            this.fieldData = Object.assign({}, data)
        },
        getCompareDate (date) {
            if (!date) return
            var d = new Date(date)
